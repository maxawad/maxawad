...
Position fix: **San Francisco, CA** — when Karl parts long enough for a sun sight; affording lee: [maxawad.com](https://maxawad.com).
...
Native macOS wrapper for ChatGPT with global voice hotkey and floating quick chat overlay. Press `Ctrl+Space` from any app to hail ChatGPT without breaking cadence. Uses your existing ChatGPT account — GPT-4o, voice mode, all Plus features.
...
macOS Cmd+Tab only switches apps. WindowSwitch shows every window with live thumbnails so you land exactly where you left off. True MRU ordering, customizable shortcut, zero config. The Alt+Tab experience macOS keeps fossilizing barnacles on the backlog.
